...
class EnhancedAdaptiveMLTradingSystem:
    ...
    def get_ensemble_predictions(self, features, symbol):
        ...
        try:
            ...
            # Create feature vector aligned with training
            feature_vector = []
            missing_features = []
            ...
            for feature_name in self.feature_columns:
                if feature_name in processed_features:
                    ...
                    value = processed_features[feature_name]
                    try:
            ...
            # Always ensure we have exactly 62 features for the scaler
            if feature_array.shape[1] != 62:
                if self.debug_mode:
                    print(f"🔧 Adjusting feature count from {feature_array.shape[1]} to 62 for scaler")
                
                if feature_array.shape[1] < 62:
                    # Pad with zeros to reach 62 features
                    padding_needed = 62 - feature_array.shape[1]
                    padding = np.zeros((feature_array.shape[0], padding_needed))
                    feature_array = np.hstack([feature_array, padding])
                else:
                    # Truncate to 62 features if we somehow have more
            ...
            for model_name, model in self.models.items():
                ...
                try:
                    # Special handling for adaptive_learner (online model)
                    if model_name == 'adaptive_learner':
                        # Use the calibrated online prediction
                        prob = self.predict_online(feature_vector)
                    else:
                        # Prepare feature array for model prediction
                        model_input = feature_array_scaled
                        
                        # Handle dimension mismatch - models expect 64 features
                        if model_input.shape[1] < 64:
                            # Pad with zeros to reach 64 features for RandomForest
                            padding = np.zeros((model_input.shape[0], 64 - model_input.shape[1]))
                            model_input = np.hstack([model_input, padding])
                        elif model_input.shape[1] > 64:
                            # Truncate to 64 features if somehow we have more
                            model_input = model_input[:, :64]
                        
                        # Use standard model prediction
                        prob = model.predict_proba(model_input)[0][1]
                        
                        # Apply timeframe-specific calibration for timeframe_ensemble model
        ...
        try:
            ...
            if hasattr(self, 'confidence_threshold'):
                ...
                if isinstance(threshold, (int, float)):
                    ...
                    # Handle dictionary threshold values
